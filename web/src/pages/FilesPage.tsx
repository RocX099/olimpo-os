...
const CONTEXT_ENTRY_LIMIT = 6;

export function limitContextEntries(entries: ManagedFileEntry[]): {
  folderEntries: ManagedFileEntry[];
  cardEntries: ManagedFileEntry[];
  hiddenCount: number;
} {
  const folders = entries.filter((entry) => entry.is_directory);
  const files = entries.filter((entry) => !entry.is_directory);
  const folderEntries = folders.slice(0, CONTEXT_ENTRY_LIMIT);
  const cardEntries = [...folders, ...files].slice(0, CONTEXT_ENTRY_LIMIT);

  return {
    folderEntries,
    cardEntries,
    hiddenCount: Math.max(0, entries.length - cardEntries.length),
  };
}
...
export default function FilesPage() {
  ...
  const limitedEntries = limitContextEntries(listing?.entries ?? []);
  const folderEntries = limitedEntries.folderEntries;
  const visibleEntries = limitedEntries.cardEntries;
  ...
  return (
    <div className="flex min-w-0 max-w-full flex-col gap-4">
      ...
      {folderEntries.length > 0 && (
        <section className="space-y-3" aria-label="Carpetas de contexto">
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {folderEntries.map((folder) => (
              <button
                ...
                className="group flex min-h-28 min-w-0 items-start gap-3 border border-border bg-background/25 p-4 text-left transition hover:border-midground/60 hover:bg-background/45"
              >
                <span className="flex h-10 w-10 shrink-0 items-center justify-center border border-border bg-background/50 text-warning">
                  <FolderOpen className="h-5 w-5" />
                </span>
                <span className="min-w-0">
                  <span className="block truncate text-sm font-semibold uppercase tracking-[0.08em] text-foreground">
                    Carpeta de {folder.name}
                  </span>
                  <span className="mt-2 block line-clamp-2 font-mono text-xs text-text-secondary">
                    Entra aqui para ver y trabajar solo el contexto de {folder.name}.
                  </span>
                </span>
              </button>
            ))}
          </div>
          {limitedEntries.hiddenCount > 0 && (
            <p className="text-xs text-text-tertiary">
              Mostrando 6 recuadros de este nivel. Entra en una carpeta para desplegar el siguiente bloque.
            </p>
          )}
        </section>
      )}
