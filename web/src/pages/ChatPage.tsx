...
function sendPantheonPreset(ws: WebSocket, preset: PantheonChatPreset, delayMs: number) {
  window.setTimeout(() => {
    if (ws.readyState !== WebSocket.OPEN) return;
    ws.send(`${preset.modelSwitchCommand}\r`);
    window.setTimeout(() => {
      if (ws.readyState === WebSocket.OPEN) {
        ws.send(preset.commandDraft);
      }
    }, 350);
  }, delayMs);
}
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const olimpoGodParam = searchParams.get("olimpoGod");
  const olimpoPreset = useMemo(
    () => (olimpoGodParam ? getPantheonChatPreset(olimpoGodParam) : null),
    [olimpoGodParam],
  );
  const olimpoPresetSentRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    olimpoPresetSentRef.current = null;
  }, [olimpoGodParam]);

  useEffect(() => {
    if (!olimpoPreset) return;
    if (olimpoPresetSentRef.current === olimpoPreset.agentId) return;
    const ws = wsRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) return;

    olimpoPresetSentRef.current = olimpoPreset.agentId;
    sendPantheonPreset(ws, olimpoPreset, 200);
  }, [olimpoPreset]);
  ...
  useEffect(() => {
    ...
    ws.onopen = () => {
      ...
      if (
        ...
        olimpoPresetSentRef.current = olimpoPreset.agentId;
        sendPantheonPreset(ws, olimpoPreset, 800);
      }
    };
  ...
  }, [channel, resumeParam, scopedProfile, reconnectNonce, olimpoPreset]);
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="grid gap-2 border border-border/50 bg-background/30 px-3 py-2 text-xs text-text-secondary lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="min-w-0">
          ...
          <span className="ml-2 font-mono [overflow-wrap:anywhere]">
            {OLLAMA_LOCAL_MODELS.join(" · ")}
          </span>
        </div>
        {olimpoPreset && (
          <div className="min-w-0 lg:text-right">
            <span className="font-semibold uppercase tracking-[0.08em] text-foreground">
              Preset {olimpoPreset.agentName}
            </span>
            <span className="ml-2 font-mono [overflow-wrap:anywhere]">
              {olimpoPreset.model}
            </span>
