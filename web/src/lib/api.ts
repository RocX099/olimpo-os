...
export const api = {
  ...
  getVoiceStatus: () => fetchJSON<VoiceStatusResponse>("/api/voice/status"),
  getVoiceConfig: () => fetchJSON<Record<string, unknown>>("/api/voice/config"),
  getVoiceLogs: () => fetchJSON<string[]>("/api/voice/logs"),
  toggleVoiceRecord: () => fetchJSON<Record<string, unknown>>("/api/voice/record/toggle", { method: "POST" }),
  toggleVoiceChat: () => fetchJSON<Record<string, unknown>>("/api/voice/chat/toggle", { method: "POST" }),
  testVoiceTts: () => fetchJSON<Record<string, unknown>>("/api/voice/tts/test", { method: "POST" }),
  /**
...
export interface VoiceStatusResponse {
  daemon?: boolean;
  recording?: boolean;
  chat_recording?: boolean;
  continuous_dictation?: boolean;
  processing?: boolean;
  message?: string;
  whisper?: boolean;
  ollama?: boolean;
  xdotool?: string;
  latest?: { text?: string; created_at?: number; duration_seconds?: number } | null;
  hotkeys?: { dictation?: string; chat?: string };
  tts?: {
    engine?: string;
    local_engine?: string;
