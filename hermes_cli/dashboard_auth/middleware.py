...
def _has_valid_internal_session_token(request: Request) -> bool:
    """Accept local service-to-service calls that carry the shared token.

    Password auth can force the gated middleware on even for loopback-only
    dashboards. VoiceOS is not a browser and cannot hold the dashboard's auth
    cookies, so it authenticates with the same private session token used by
    the local dashboard runtime.
    """
    expected = os.environ.get("HERMES_DASHBOARD_SESSION_TOKEN", "")
...
async def gated_auth_middleware(
    ...
    if _path_is_public(path):
        return await call_next(request)
    if _has_valid_internal_session_token(request):
        request.state.session = {"provider": "internal", "user_id": "local-service"}
        return await call_next(request)
