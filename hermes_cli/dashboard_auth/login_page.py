...
Visual styling mirrors the Olimpo dashboard base theme: dark green
canvas, cream text, uppercase + wide-tracking brand chrome, and compact
credential controls. Fonts are served
...
<style>
  ...
  :root {{
    ...
    --midground: #ffe6cb;
    --foreground: #ffffff;
    --hairline: color-mix(in srgb, #ffe6cb 18%, transparent);
    --hairline-strong: color-mix(in srgb, #ffe6cb 35%, transparent);
  }}
  ...
  *, *::before, *::after {{ box-sizing: border-box; }}
  ...
  /* Subtle dot-grid backdrop — same dark OS canvas as the dashboard. */
  body {{
    background-image:
      radial-gradient(
        ellipse at top,
        color-mix(in srgb, var(--midground) 5%, transparent) 0%,
        transparent 55%
      ),
  ...
  .brand {{
    ...
    color: color-mix(in srgb, var(--midground) 82%, transparent);
  }}
  .brand .dot {{
  ...
  .provider-btn {{
    ...
    text-align: center;
    background: color-mix(in srgb, var(--midground) 10%, var(--background-base));
    color: var(--midground);
  ...
  .field-input {{
    ...
    background: color-mix(in srgb, #000000 18%, var(--background-base));
    color: var(--foreground);
    border: 1px solid var(--hairline-strong);
    border-radius: 0;
    font-family: 'Collapse', sans-serif;
...
<body>
<main>
  <div class="brand">Olimpo<span class="dot"></span>OS</div>
  <div class="card">
    <h1>Bienvenido al Olimpo</h1>
    <p class="subtitle">Sr. Roca, inicia sesión para entrar al panel privado.</p>
    <div class="provider-list">
{provider_buttons}
    </div>
  </div>
  <footer>
    <span class="sep"></span>VPS privada &middot; Auth requerida<span class="sep"></span>
  </footer>
</main>
{password_script}
</body>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Login unavailable — Olimpo OS</title>
<style>
  @font-face {
    font-family: 'Collapse';
    font-style: normal;
