...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    """
    force_auth = os.environ.get("HERMES_DASHBOARD_FORCE_AUTH", "").strip().lower()
    if force_auth in {"1", "true", "yes", "on"}:
        return True
...
_VOICEOS_BASE_URL = "http://127.0.0.1:8080"
_VOICEOS_ROUTE_MAP = {
    "status": ("GET", "/api/status"),
    "config": ("GET", "/api/config"),
    "logs": ("GET", "/api/logs"),
    "record_toggle": ("POST", "/api/record/toggle"),
    "chat_toggle": ("POST", "/api/chat/toggle"),
...
_VOICE_OLLAMA_URL = "http://127.0.0.1:11434/api/chat"
_VOICE_OLLAMA_MODEL = os.environ.get("HERMES_VOICE_OLLAMA_MODEL", "llama3.1:8b").strip() or "llama3.1:8b"
_VOICE_DEEP_MODEL = os.environ.get("HERMES_VOICE_DEEP_MODEL", "huihui_ai/deepseek-r1-abliterated:8b").strip() or "huihui_ai/deepseek-r1-abliterated:8b"
_VOICE_DEEP_TRIGGER = "contestame con pensamiento profundo"
_VOICE_OLIMPO_ACTION_POLICY = (
    "Lectura amplia de Olimpo OS, diagnostico seguro del sistema y escritura de artefactos "
    "en el workspace de Olimpo/Hermes. Puede crear archivos, prompts, textos y planes cuando "
    "Oriol lo pida por voz. Eliminacion, borrado y destruccion siguen bloqueados."
)
_VOICE_WORKSPACE_ROOT = Path("/home/oriol/AI-Workstation")
_VOICE_ARTIFACT_DIR = _VOICE_WORKSPACE_ROOT / "prompts" / "voice"
_VOICE_OLIMPO_ACCESS_MODULES = [
    "Inicio",
    "Olimpo Chat",
    "Contexto",
    "Modelos",
...
_VOICE_OLIMPO_MODULES: dict[str, dict[str, Any]] = {
    "inicio": {
        "title": "Inicio",
        "route": "/olimpo",
        "summary": "Mapa principal del Olimpo OS y entrada a todas las salas.",
        "keywords": ("inicio", "home", "dashboard", "olimpo", "olympo"),
    },
    "chat": {
        "title": "Olimpo Chat",
        "route": "/chat",
        "summary": "Conversacion con perfil, prompt, modelo y contexto elegidos.",
        "keywords": ("chat", "olimpo chat", "limpio chat", "airmessage", "ai message", "mensaje"),
    },
    "contexto": {
        "title": "Contexto",
        "route": "/contexto",
        "summary": "Memoria, carpetas, sesiones, conocimiento y contexto de proyectos.",
        "keywords": ("contexto", "memoria", "rag", "documento", "proyecto", "sesion", "historial"),
    },
    "modelos": {
        "title": "Modelos",
        "route": "/modelos",
        "summary": "Modelos locales Ollama y proveedores externos disponibles.",
        "keywords": ("modelo", "modelos", "llama", "deepseek", "qwen", "ollama", "openai", "gpt", "dipsy"),
    },
    "errores": {
        "title": "Errores",
        "route": "/errores",
        "summary": "Logs, errores reales, fallos y diagnostico del OS.",
        "keywords": ("error", "errores", "log", "logs", "fallo", "bug", "crash", "sentry"),
    },
    "cron": {
        "title": "Cron",
        "route": "/cron",
        "summary": "Rutinas programadas como mini programas con modelo y fallback.",
        "keywords": ("cron", "rutina", "rutinas", "programa", "programar", "agenda", "periodico"),
    },
    "skills": {
        "title": "Skills",
        "route": "/skills",
        "summary": "Skills, plugins, MCPs y herramientas disponibles.",
        "keywords": ("skill", "skills", "plugin", "plugins", "mcp", "herramienta", "herramientas"),
    },
    "voice": {
        "title": "VOICE",
        "route": "/voice",
        "summary": "Entrada por voz, dictado, conversacion, TTS y router Olimpo.",
        "keywords": ("voice", "voz", "audio", "hablar", "microfono", "transcribir", "tts"),
    },
    "pairing": {
        "title": "Pairing",
        "route": "/pairing",
        "summary": "Conexion movil, QR, WhatsApp, Telegram y canales remotos.",
        "keywords": ("pairing", "whatsapp", "telegram", "movil", "qr", "canal", "conexion", "boys"),
    },
    "radar": {
        "title": "Radar",
        "route": "/radar",
        "summary": "Vigilancia, busqueda recurrente y salida a Contexto/Radar.",
        "keywords": ("radar", "vigilar", "vigilancia", "monitor", "rastrear", "buscar"),
    },
    "panteon": {
        "title": "Panteon",
        "route": "/panteon",
        "summary": "Dioses, prompts, roles y modelos especializados del Olimpo.",
        "keywords": ("panteon", "pantheon", "dios", "dioses", "predilecto", "zeus", "hera", "poseidon", "atenea", "apolo", "artemisa", "ares", "afrodita"),
    },
    "keys": {
        "title": "Keys",
        "route": "/keys",
        "summary": "Estado no secreto de claves, proveedores y configuracion sensible.",
        "keywords": ("key", "keys", "clave", "claves", "api key", "token", "secreto", "secrets"),
    },
    "sistema": {
        "title": "Sistema",
        "route": "/sistema",
        "summary": "Estado del sistema, servicios, permisos y salud operativa.",
        "keywords": ("sistema", "system", "estado", "servicio", "systemctl", "permiso", "permisos", "login", "hardware", "cpu", "gpu", "ram", "disco"),
    },
    "kanban": {
        "title": "Kanban",
        "route": "/kanban",
        "summary": "Tareas, tableros, misiones, bloqueos y seguimiento operativo.",
        "keywords": ("kanban", "tarea", "tareas", "board", "tablero", "pendiente", "mision", "misiones"),
    },
    "archimedes": {
        "title": "Archimedes",
        "route": "/archimedes",
        "summary": "Analisis tecnico, arquitectura, calculo y razonamiento de sistemas.",
        "keywords": ("archimedes", "arquimedes", "arquitectura", "calculo", "analisis tecnico"),
    },
}
_VOICE_PANTHEON_AGENTS: dict[str, dict[str, Any]] = {
    "zeus": {
        "id": "zeus",
        "name": "Zeus",
        "role": "Supremo",
        ...
        "keywords": ("zeus", "profundo", "decision", "decide", "critico", "prioridad", "mando"),
        "prompt": (
            "Actua como Zeus, soberano del Olimpo OS. Usa pensamiento profundo antes de responder: "
            "prioridades, riesgos y consecuencias. Responde con criterio firme, ordena lo importante, "
            "elimina ruido, marca una decision clara y termina con el siguiente paso verificable para Oriol."
        ),
    },
    "hera": {
        "id": "hera",
        ...
        "role": "Gobierno",
        "model": "llama3.1:8b",
        "duty": "Configuracion, perfiles, orden interno y reglas del sistema.",
        "keywords": ("hera", "config", "configuracion", "permiso", "permisos", "seguridad", "key", "login", "orden", "perfil"),
        "prompt": (
            "Actua como Hera, gobierno interno del Olimpo OS. Revisa configuracion, permisos, perfiles, "
            "seguridad y orden del sistema. Protege lo privado y deja una decision administrativa clara."
        ),
    },
    "poseidon": {
        "id": "poseidon",
        ...
        "keywords": ("poseidon", "pairing", "whatsapp", "telegram", "canal", "canales", "movil", "conexion", "qr", "boys"),
        "prompt": (
            "Actua como Poseidon, guardian de conexiones. Piensa en canales, pairing movil, WhatsApp, "
            "Telegram, LAN, VPN y relay seguro. Prioriza estabilidad, tokens temporales y pasos concretos."
        ),
    },
    "atenea": {
        "id": "atenea",
        ...
        "keywords": ("atenea", "plan", "estrategia", "arquitectura", "contexto", "investiga", "estudio", "criterio"),
        "prompt": (
            "Actua como Atenea, estratega del Olimpo OS. Convierte ideas confusas en arquitectura, plan "
            "y criterios. Separa lo urgente de lo importante, identifica dependencias y entrega una secuencia ejecutable."
        ),
    },
    "apolo": {
        "id": "apolo",
        ...
        "role": "Claridad",
        "model": "qwen3.5:4b",
        "duty": "Analisis visual, creatividad, presentacion, estilo, logos y explicacion limpia.",
        "keywords": ("apolo", "logo", "marca", "visual", "imagen", "prompt", "estilo", "presentacion", "ui", "diseno", "diseño", "creativo", "peli"),
        "prompt": (
            "Actua como Apolo, claridad y presentacion. Convierte informacion tecnica o caotica en una salida "
            "visual, limpia y util. Para logos o imagenes, entrega un prompt usable, con direccion artistica, "
            "composicion, colores, materiales y restricciones."
        ),
    },
    "artemisa": {
        "id": "artemisa",
        ...
        "keywords": ("artemisa", "cron", "rutina", "objetivo", "foco", "seguimiento", "radar", "vigila", "alerta"),
        "prompt": (
            "Actua como Artemisa, vigilia y foco. Rastrea objetivos, rutinas y amenazas de dispersion. "
            "Resume estado, detecta bloqueos y propone una accion pequena pero inmediata."
        ),
    },
    "ares": {
        "id": "ares",
        ...
        "keywords": ("ares", "codigo", "implementa", "programa", "arregla", "bug", "test", "deploy", "comando", "terminal", "ejecuta"),
        "prompt": (
            "Actua como Ares, ejecucion tecnica. Toma el objetivo y conviertelo en cambios concretos, comandos, "
            "pruebas y verificacion. No ejecutes ni prometas acciones destructivas."
        ),
    },
    "afrodita": {
        "id": "afrodita",
        ...
        "keywords": ("afrodita", "mensaje", "tono", "copy", "comunicacion", "humano", "cliente", "texto", "monologo", "monólogo"),
        "prompt": (
            "Actua como Afrodita, relacion y lenguaje humano. Pule tono, mensajes, energia visual y comunicacion "
            "delicada sin perder precision ni decision."
        ),
    },
}
_VOICE_WRITE_KEYWORDS = (
    "guarda",
    "guardar",
    "crea",
    "creame",
...
_VOICE_DELETE_KEYWORDS = (
    ...
    "destruye",
    "destruir",
    "quita definitivamente",
    "purga",
    "wipe",
    "delete",
    "remove",
)
_VOICE_ARTIFACT_KEYWORDS = (
    "prompt",
    "logo",
    "texto",
    "monologo",
...
_VOICE_WEB_KEYWORDS = ("busca", "buscar", "investiga", "internet", "web", "repo", "github", "noticias")
_VOICE_HARDWARE_KEYWORDS = (
    "hardware",
    "cpu",
    "gpu",
    "ram",
...
def _sanitize_voice_payload(value: Any) -> Any:
    if isinstance(value, dict):
        sanitized = {}
        for key, item in value.items():
            if str(key) in _VOICE_SECRET_KEYS or "api_key" in str(key).lower():
                continue
            sanitized[key] = _sanitize_voice_payload(item)
        return sanitized
    if isinstance(value, list):
        return [_sanitize_voice_payload(item) for item in value]
    return value


def _voiceos_request(method: str, path: str, json_body: dict[str, Any] | None = None) -> Any:
    parsed = urllib.parse.urlparse(_VOICEOS_BASE_URL)
    if parsed.hostname != "127.0.0.1":
        raise HTTPException(status_code=500, detail="VoiceOS proxy must target loopback only.")
    url = urllib.parse.urljoin(_VOICEOS_BASE_URL, path)
    data = None
    ...
    try:
        ...
        raise HTTPException(status_code=503, detail=f"VoiceOS no esta activo en 127.0.0.1:8080: {exc}")
    if not raw:
        return {}
    try:
        return json.loads(raw.decode("utf-8"))
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=502, detail=f"VoiceOS devolvio JSON invalido: {exc}")


def _voice_normalize_command(text: str) -> str:
    normalized = unicodedata.normalize("NFD", text.lower())
    return "".join(ch for ch in normalized if unicodedata.category(ch) != "Mn")


def _voice_uses_deep_thinking(prompt: str) -> bool:
    normalized = _voice_normalize_command(prompt)
    triggers = (
        _VOICE_DEEP_TRIGGER,
        "respondeme con pensamiento profundo",
        "responde con pensamiento profundo",
        "piensa profundo",
        "piensalo profundo",
        "analizalo profundo",
    )
...
def _voice_strip_deep_trigger(prompt: str) -> str:
    cleaned = _voice_normalize_command(prompt)
    if _VOICE_DEEP_TRIGGER in cleaned:
        start = cleaned.find(_VOICE_DEEP_TRIGGER)
        end = start + len(_VOICE_DEEP_TRIGGER)
        original = prompt[:start] + prompt[end:]
        return original.strip(" ,.:;-") or prompt
    return prompt


def _voice_has_any(normalized_prompt: str, keywords: tuple[str, ...]) -> bool:
    for keyword in keywords:
        normalized_keyword = _voice_normalize_command(keyword)
        if not normalized_keyword:
            continue
        if " " not in normalized_keyword and normalized_keyword.isalnum():
...
def _voice_select_modules(normalized_prompt: str) -> list[str]:
    selected = [
        module_id
        for module_id, module in _VOICE_OLIMPO_MODULES.items()
        if _voice_has_any(normalized_prompt, tuple(module.get("keywords") or ()))
    ]
    if "panteon" in selected and "modelos" not in selected:
        selected.append("modelos")
    if not selected:
        selected = ["inicio", "chat", "contexto"]
    return selected


def _voice_select_god(normalized_prompt: str) -> dict[str, Any] | None:
    for agent in _VOICE_PANTHEON_AGENTS.values():
        if _voice_normalize_command(str(agent["name"])) in normalized_prompt:
            return dict(agent)
    ...
    for agent in _VOICE_PANTHEON_AGENTS.values():
        score = sum(
            1
            for keyword in tuple(agent.get("keywords") or ())
            if _voice_has_any(normalized_prompt, (str(keyword),))
        )
        if score:
...
def _voice_route_request(prompt: str) -> dict[str, Any]:
    normalized = _voice_normalize_command(prompt)
    modules = _voice_select_modules(normalized)
    selected_god = _voice_select_god(normalized)
    if "panteon" in modules and selected_god is None:
        selected_god = dict(_VOICE_PANTHEON_AGENTS["zeus"])

    deletion_blocked = _voice_has_any(normalized, _VOICE_DELETE_KEYWORDS)
    write_requires_permission = (
        not deletion_blocked and _voice_has_any(normalized, _VOICE_WRITE_KEYWORDS)
    )
    needs_web = _voice_has_any(normalized, _VOICE_WEB_KEYWORDS)
    needs_hardware = _voice_has_any(normalized, _VOICE_HARDWARE_KEYWORDS)
    short_answer = _voice_has_any(normalized, _VOICE_SHORT_ANSWER_KEYWORDS)
    deep = _voice_uses_deep_thinking(prompt)
    artifact = _voice_has_any(normalized, _VOICE_ARTIFACT_KEYWORDS)

    intent: list[str] = []
    if deletion_blocked:
        intent.append("delete_blocked")
    if write_requires_permission:
        intent.append("write_request")
    if needs_web:
        intent.append("search")
    if needs_hardware:
        intent.append("hardware")
    if artifact:
        intent.append("artifact")
    if selected_god is not None:
        intent.append("pantheon")
    if deep:
        intent.append("deep")
    if not intent:
        intent.append("conversation")

    model = str((selected_god or {}).get("model") or _VOICE_OLLAMA_MODEL)
    return {
        "intent": intent,
        "modules": modules,
        "module_details": [
            {
                "id": module_id,
                "title": str(_VOICE_OLIMPO_MODULES[module_id]["title"]),
                "route": str(_VOICE_OLIMPO_MODULES[module_id]["route"]),
                "summary": str(_VOICE_OLIMPO_MODULES[module_id]["summary"]),
            }
            for module_id in modules
            if module_id in _VOICE_OLIMPO_MODULES
        ],
        ...
        "deep": deep,
        "write_requires_permission": write_requires_permission,
        "deletion_blocked": deletion_blocked,
        "access_modules": list(_VOICE_OLIMPO_ACCESS_MODULES),
        "permissions": _VOICE_OLIMPO_ACTION_POLICY,
    }


def _voice_safe_slug(text: str) -> str:
    normalized = _voice_normalize_command(text)
    slug = re.sub(r"[^a-z0-9]+", "-", normalized).strip("-")
    return (slug[:70] or "voice-artifact").strip("-")


def _voice_run_safe_command(command: list[str], *, timeout: int = 4) -> dict[str, Any]:
    try:
        result = subprocess.run(
            command,
            cwd=str(_VOICE_WORKSPACE_ROOT),
            text=True,
        ...
        return {"command": command, "ok": False, "output": "comando no disponible"}
    except subprocess.TimeoutExpired:
        return {"command": command, "ok": False, "output": "timeout"}
...
def _voice_collect_hardware_context() -> dict[str, Any]:
    commands = [
        ["uname", "-a"],
        ["lscpu"],
        ["free", "-h"],
    ...
    results = [_voice_run_safe_command(command) for command in commands]
    readable = []
    for item in results:
        readable.append(
            "$ " + " ".join(item["command"]) + "\n" + str(item.get("output") or "sin salida")
        )
    return {
        "type": "hardware",
        "results": results,
...
def _voice_write_artifact(prompt: str, answer: str, route: dict[str, Any]) -> dict[str, Any] | None:
    if route.get("deletion_blocked"):
        return None
    if "artifact" not in (route.get("intent") or []) or not route.get("write_requires_permission"):
        return None

    _VOICE_ARTIFACT_DIR.mkdir(parents=True, exist_ok=True)
    slug = _voice_safe_slug(prompt)
    stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
    path = (_VOICE_ARTIFACT_DIR / f"{stamp}-{slug}.md").resolve()
    root = _VOICE_WORKSPACE_ROOT.resolve()
    if root not in path.parents:
        raise HTTPException(status_code=500, detail="Ruta de artefacto fuera del workspace.")

    selected_god = route.get("selected_god") or {}
    content = "\n".join(
        [
            "# Artefacto VOICE Olimpo",
            "",
            f"- Creado: {datetime.now(timezone.utc).isoformat()}",
            f"- Dios: {selected_god.get('name') or 'Olimpo'}",
            f"- Modelo: {route.get('model')}",
            f"- Modulos: {', '.join(route.get('modules') or [])}",
            "",
            "## Peticion",
            "",
            prompt.strip(),
...
def _voice_execution_line(route: dict[str, Any]) -> str:
    selected_god = route.get("selected_god") or {}
    if selected_god:
        return (
            f"Ruta real de ejecucion: {selected_god.get('name')} "
            f"({selected_god.get('role')}) con {route.get('model')}."
        )
    return f"Ruta real de ejecucion: Olimpo OS con {route.get('model')}."


def _voice_compact_short_answer(answer: str) -> str:
    cleaned_lines = []
    for line in answer.splitlines():
        normalized = _voice_normalize_command(line)
        if "dios/modelo" in normalized or "modelo utilizado" in normalized:
            continue
        if normalized.startswith("prompt:") or normalized.startswith("modelo:"):
            continue
        cleaned_lines.append(line.strip())
    cleaned = " ".join(line for line in cleaned_lines if line).strip()
    parts = re.split(r"(?<=[.!?])\s+", cleaned)
    compact = " ".join(part for part in parts[:2] if part).strip()
    return compact or cleaned or answer.strip()


def _voice_build_olimpo_context(route: dict[str, Any]) -> str:
    module_lines = [
        f"- {item['title']} ({item['route']}): {item['summary']}"
        for item in route.get("module_details") or []
    ]
    selected_god = route.get("selected_god") or {}
    include_full_pantheon = (
        "panteon" in (route.get("modules") or [])
        or "artifact" in (route.get("intent") or [])
        or bool(selected_god)
    )
    if include_full_pantheon:
        agent_lines = [
            f"- {agent['name']} ({agent['role']}): {agent['duty']} Modelo: {agent['model']}."
            for agent in _VOICE_PANTHEON_AGENTS.values()
        ]
    else:
        agent_lines = []
    selected_line = (
        f"Dios seleccionado: {selected_god.get('name')} ({selected_god.get('role')}) con {route.get('model')}."
        if selected_god
        else f"No hay dios seleccionado para esta peticion; respuesta base con {route.get('model')}."
    )
    access_line = (
        f"Acceso disponible: {', '.join(route.get('access_modules') or _VOICE_OLIMPO_ACCESS_MODULES)}."
        if include_full_pantheon or route.get("needs_hardware") or route.get("write_requires_permission")
        else "Acceso disponible: lectura de Olimpo OS y respuesta conversacional."
    )
    web_line = (
        "Web: la peticion parece necesitar busqueda externa; si no hay herramienta web activa, dilo y no inventes."
        if route.get("needs_web")
        else "Web: no necesaria salvo que Oriol lo pida o falten datos actuales."
    )
    write_line = (
        "Escritura: puedes crear archivos reales dentro de /home/oriol/AI-Workstation cuando Oriol lo pida por voz."
        if route.get("write_requires_permission")
        else "Escritura: disponible para artefactos si Oriol pide crear o guardar algo."
    )
    return "\n".join(
        [
            "Contexto real de Olimpo OS para VOICE:",
            access_line,
            f"Politica de permisos: {route.get('permissions') or _VOICE_OLIMPO_ACTION_POLICY}",
            selected_line,
            "Modulos relevantes:",
            *module_lines,
            ("Panteon disponible:" if agent_lines else "Panteon: no consultado para esta peticion."),
            *agent_lines,
            web_line,
            (
                "Herramientas: esta peticion pide hardware; usa la salida real de comandos seguros si esta disponible."
                if route.get("needs_hardware")
                else "Herramientas: puedes usar diagnostico seguro del sistema cuando la peticion lo requiera."
            ),
            (
                "Longitud: Oriol pidio respuesta corta; responde en una o dos frases, sin listas ni secciones."
                if route.get("short_answer")
                else "Longitud: responde con el detalle justo para que sea util por voz."
            ),
            write_line,
            (
                "Eliminacion: bloqueada. No propongas borrar ni ejecutar comandos destructivos; explica que Oriol debe hacerlo manualmente."
                if route.get("deletion_blocked")
                else "Eliminacion: bloqueada por defecto."
            ),
            "Contrato de verdad: no digas que has consultado Panteon, elegido un dios, ejecutado una herramienta, escrito un archivo o mirado el sistema salvo que aparezca explicitamente en este contexto o en herramientas reales.",
            "Contrato de respuesta: usa solo el contexto anterior, di 'no tengo ese dato' si falta informacion, entrega el artefacto si se pide, y menciona dios/modelo solo si hay dios seleccionado.",
            "Ubicacion de la respuesta: si se crea archivo real, devuelve su ruta exacta. Si no se crea archivo, di que queda en esta respuesta de VOICE.",
        ]
    )


def _voice_public_route(route: dict[str, Any]) -> dict[str, Any]:
    selected_god = route.get("selected_god")
    return {
        "intent": route.get("intent") or [],
        "modules": route.get("modules") or [],
        "module_details": route.get("module_details") or [],
        "selected_god": (
            {
                "id": selected_god.get("id"),
                "name": selected_god.get("name"),
                "role": selected_god.get("role"),
                "model": selected_god.get("model"),
                "duty": selected_god.get("duty"),
            }
            if isinstance(selected_god, dict)
            else None
        ),
        "model": route.get("model"),
        "needs_web": bool(route.get("needs_web")),
        "needs_hardware": bool(route.get("needs_hardware")),
        "short_answer": bool(route.get("short_answer")),
        "write_requires_permission": bool(route.get("write_requires_permission")),
        "deletion_blocked": bool(route.get("deletion_blocked")),
        "permissions": route.get("permissions"),
    }


def _voice_answer_looks_like_reasoning(answer: str) -> bool:
    normalized = _voice_normalize_command(answer)
    reasoning_markers = (
        "thinking process",
        "analisis interno",
...
def _voice_answer_claims_write(answer: str) -> bool:
    normalized = _voice_normalize_command(answer)
    write_claims = (
        "guardo",
        "guardado",
    ...
    return any(claim in normalized for claim in write_claims)


def _voice_ollama_chat(model: str, messages: list[dict[str, str]], *, timeout: int, temperature: float, num_predict: int) -> str:
    system_prompt = (
        "Eres Olimpo OS, el asistente operativo local de Oriol. "
        f"Tu modelo principal de conversacion por voz es {_VOICE_OLLAMA_MODEL}; si se activa pensamiento profundo, revisas con {_VOICE_DEEP_MODEL} y sintetizas con {_VOICE_OLLAMA_MODEL}. "
        "Responde en espanol, de forma breve y accionable. "
        "Contesta directamente, sin markdown, sin comillas envolventes y sin prefacios. "
        "No digas que has transcrito texto ni que eres un modo de dictado. "
        "Trata la entrada como una pregunta hablada y contesta como conversacion. "
        "Nunca muestres el proceso de pensamiento, borradores internos ni analisis paso a paso. "
        "No inventes acciones, lecturas, archivos, rutas, dioses seleccionados, herramientas usadas ni estado del sistema. "
        "Si no hay herramienta real o dato real en el contexto, di que no lo sabes o que no se ha ejecutado. "
        "Puedes ayudar a operar el OS y preparar acciones cuando el sistema tenga rutas seguras para ello. "
        "No elimines, borres, destruyas ni propongas ejecutar acciones destructivas; para borrar pide confirmacion humana y explica el riesgo. "
        "Si faltan datos o permisos, pide exactamente lo necesario."
    )
    payload = {
        "model": model,
        "stream": False,
    ...
    try:
        ...
        detail = exc.read().decode("utf-8", errors="replace")
        raise HTTPException(status_code=502, detail=f"Ollama devolvio HTTP {exc.code}: {detail}")
    except OSError as exc:
        raise HTTPException(status_code=503, detail=f"Ollama no esta activo en 127.0.0.1:11434: {exc}")
    try:
        data = json.loads(raw.decode("utf-8"))
    except json.JSONDecodeError as exc:
        raise HTTPException(status_code=502, detail=f"Ollama devolvio JSON invalido: {exc}")
    message = data.get("message") or {}
    answer = (message.get("content") or message.get("thinking") or data.get("response") or "").strip()
    if not answer:
        raise HTTPException(status_code=502, detail="Ollama no devolvio contenido hablable.")
    return answer


def _voice_ollama_answer(prompt: str) -> dict[str, Any]:
    route = _voice_route_request(prompt)
    if route["deletion_blocked"]:
        selected_god = route.get("selected_god") or _VOICE_PANTHEON_AGENTS["hera"]
        return {
            "ok": True,
            "answer": (
                "No voy a eliminar ni borrar nada desde VOICE. Te puedo preparar un plan seguro de limpieza "
                f"en {', '.join(item['title'] for item in route['module_details'])}, pero la eliminacion la haces tu manualmente. "
                f"Ruta aplicada: {selected_god['name']} con politica de permisos de Olimpo."
            ),
            "provider": "olimpo-router",
            "model": selected_god["model"],
            "mode": "blocked",
            "models": [selected_god["model"]],
            "routing": _voice_public_route(route),
        }
    ...
    selected_god = route.get("selected_god") or {}
    if selected_god:
        system_prompt = f"{system_prompt}\n\n{selected_god.get('prompt')}"
    olimpo_context = _voice_build_olimpo_context(route)
    tool_results: list[dict[str, Any]] = []
    if route.get("needs_hardware"):
        hardware = _voice_collect_hardware_context()
        tool_results.append(hardware)
        olimpo_context = (
            f"{olimpo_context}\n\n"
            "DATOS REALES DE HARDWARE Y SISTEMA:\n"
            f"{hardware['context']}"
        )
    selected_model = str(route.get("model") or _VOICE_OLLAMA_MODEL)
    if _voice_uses_deep_thinking(prompt):
        user_prompt = _voice_strip_deep_trigger(prompt)
        critique = _voice_ollama_chat(
            _VOICE_DEEP_MODEL,
            [
                {
                    "role": "system",
                    "content": (
                        "Eres el razonador interno de Olimpo OS. Analiza la peticion de Oriol con cuidado, "
                        "detecta riesgos, dependencias y pasos utiles. No redactes la respuesta final; entrega criterios concisos. "
                        "No autorices acciones de borrado o destruccion.\n\n"
                        f"{olimpo_context}"
                    ),
                },
        ...
        answer = _voice_ollama_chat(
            ...
            [
                {"role": "system", "content": system_prompt},
                {
                    ...
                    "content": (
                        f"{olimpo_context}\n\n"
                        f"Pregunta hablada de Oriol: {user_prompt}\n\n"
                        f"Analisis interno de {_VOICE_DEEP_MODEL}: {critique}\n\n"
                        "Da la respuesta final hablable, clara y no demasiado larga."
                    ),
                },
            ],
            timeout=60,
        ...
        if _voice_answer_looks_like_reasoning(answer):
            answer = _voice_ollama_chat(
                _VOICE_OLLAMA_MODEL,
                [
                    {"role": "system", "content": system_prompt},
                    {
                        ...
                        "content": (
                            f"{olimpo_context}\n\n"
                            f"Pregunta hablada de Oriol: {user_prompt}\n\n"
                            f"Borrador interno descartado: {answer}\n\n"
                            "Redacta solo la respuesta final hablable en espanol. No muestres razonamiento interno."
                        ),
                    },
                ],
                timeout=45,
        ...
        if route.get("short_answer"):
            answer = _voice_compact_short_answer(answer)
        artifact = _voice_write_artifact(user_prompt, answer, route)
        if artifact:
            answer = f"{answer}\n\n{_voice_execution_line(route)}\nHe creado el archivo aqui: {artifact['path']}"
        return {
            "ok": True,
            "answer": answer,
            "provider": "ollama",
            ...
            "mode": "deep",
            "models": [_VOICE_DEEP_MODEL, selected_model],
            "routing": _voice_public_route(route),
            "tools": tool_results,
            "artifact": artifact,
        }
    ...
    answer = _voice_ollama_chat(
        ...
        [
            {"role": "system", "content": system_prompt},
            {
                ...
                "content": (
                    f"{olimpo_context}\n\n"
                    f"Pregunta hablada de Oriol: {prompt}\n\n"
                    "Responde usando el modulo, dios y modelo elegidos. Si preparas un prompt o plan, entregalo en esta respuesta. "
                    "Si la peticion requiere guardar, modificar o ejecutar, deja claro que queda pendiente de permiso."
                ),
            },
        ],
        timeout=60,
        temperature=0.3 if route.get("short_answer") else 0.4,
        num_predict=90 if route.get("short_answer") else 360,
    )
            ...
            [
                {"role": "system", "content": system_prompt},
                {
                    ...
                    "content": (
                        f"{olimpo_context}\n\n"
                        f"Pregunta hablada de Oriol: {prompt}\n\n"
                        f"Borrador interno descartado de {selected_model}: {answer}\n\n"
                        "Redacta solo la respuesta final en espanol. No menciones el thinking ni el borrador. "
                        "Si era un prompt de logo, entrega el prompt completo y di brevemente que usaste Apolo. "
                        "Di que queda en esta respuesta de VOICE; no digas que se ha guardado en ningun sitio."
                    ),
                },
            ],
            timeout=45,
            temperature=0.3,
            num_predict=90 if route.get("short_answer") else 260,
        )
        models.append(_VOICE_OLLAMA_MODEL)
    artifact = _voice_write_artifact(prompt, answer, route)
    if artifact:
        answer = f"{answer}\n\n{_voice_execution_line(route)}\nHe creado el archivo aqui: {artifact['path']}"
    elif route["write_requires_permission"] and _voice_answer_claims_write(answer):
        god_name = str((route.get("selected_god") or {}).get("name") or "Olimpo")
        answer = (
            "He preparado el contenido en esta respuesta de VOICE, pero no he creado archivo porque no era un artefacto seguro de workspace. "
            f"Ruta aplicada: {god_name} con {selected_model}."
        )
    elif route.get("short_answer"):
        answer = _voice_compact_short_answer(answer)
    return {
        "ok": True,
        "answer": answer,
        "provider": "ollama",
        "model": selected_model,
        "mode": "deep" if route["deep"] else "normal",
        "models": models,
        "routing": _voice_public_route(route),
        "tools": tool_results,
        "artifact": artifact,
...
async def security_headers_middleware(request: Request, call_next):
    """Apply browser hardening headers to dashboard responses."""
...
def _voice_proxy(route: str, payload: dict[str, Any] | None = None) -> Any:
    method, path = _VOICEOS_ROUTE_MAP[route]
    body = _voiceos_request(method, path, payload)
    sanitized = _sanitize_voice_payload(body)
    if isinstance(sanitized, dict):
        sanitized.setdefault("elevenlabs_configured", bool(os.environ.get("ELEVENLABS_API_KEY")))
        sanitized.setdefault(
            "conversation",
            {
                "model": _VOICE_OLLAMA_MODEL,
                "deep_model": _VOICE_DEEP_MODEL,
                "deep_trigger": "Contéstame con pensamiento profundo",
                "permissions": _VOICE_OLIMPO_ACTION_POLICY,
                "context_router": "Olimpo VOICE clasifica intencion, modulo, dios y modelo antes de responder.",
                "access_modules": list(_VOICE_OLIMPO_ACCESS_MODULES),
                "pantheon": [
                    {
                        "id": agent["id"],
...
async def voice_status():
    return _voice_proxy("status")


@app.get("/api/voice/config")
async def voice_config():
    return _voice_proxy("config")


@app.get("/api/voice/logs")
async def voice_logs():
    return _voice_proxy("logs")


@app.post("/api/voice/record/toggle")
async def voice_record_toggle():
    return _voice_proxy("record_toggle")


@app.post("/api/voice/chat/toggle")
async def voice_chat_toggle():
    return _voice_proxy("chat_toggle")


@app.post("/api/voice/tts/test")
async def voice_tts_test():
    answer = await asyncio.to_thread(_voice_ollama_answer, "Di una frase corta para probar la voz de Olimpo OS.")
    speak = _voice_proxy("tts_speak", {"text": answer["answer"]})
    return {**answer, "tts": speak}


@app.post("/api/voice/ask")
async def voice_ask(payload: VoiceAskPayload):
    prompt = payload.prompt.strip()
    if not prompt:
        raise HTTPException(status_code=400, detail="Prompt vacio.")
    answer = await asyncio.to_thread(_voice_ollama_answer, prompt)
    answer["source"] = payload.source
    return answer
