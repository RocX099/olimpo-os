...
def test_gated_voice_internal_token_reaches_voice_ask(gated_app, monkeypatch):
    """VoiceOS talks to Olimpo through the shared internal session token.

    Password auth forces ``app.state.auth_required=True`` even on loopback,
    so the cookie gate must still accept the private
    ``X-Hermes-Session-Token`` for local service-to-service calls. Otherwise
    the Ctrl+Alt+Space flow records and transcribes, but Hermes rejects the
    actual question with HTTP 401 before Olimpo can answer.
    """

    def fake_voice_answer(prompt: str):
        return {"ok": True, "answer": f"respuesta a {prompt}", "provider": "test"}

    monkeypatch.setattr(web_server, "_voice_ollama_answer", fake_voice_answer)
    r = gated_app.post(
        "/api/voice/ask",
        headers={"X-Hermes-Session-Token": web_server._SESSION_TOKEN},
        json={"prompt": "hola", "source": "voiceos"},
    )

    assert r.status_code == 200, r.text
    assert r.json()["answer"] == "respuesta a hola"
