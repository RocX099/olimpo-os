...
def test_dashboard_responses_include_security_headers(client_loopback):
    r = client_loopback.get("/api/status")
    assert r.status_code == 200
...
def test_start_server_surfaces_basic_skip_reason_when_unconfigured(monkeypatch):
    from hermes_cli.dashboard_auth import clear_providers
    from plugins.dashboard_auth import basic as basic_plugin
    ...
    basic_plugin.register(MagicMock())
    assert "HERMES_DASHBOARD_BASIC_AUTH_USERNAME" in basic_plugin.LAST_SKIP_REASON
    ...
    msg = str(exc_info.value)
    assert "HERMES_DASHBOARD_BASIC_AUTH_USERNAME" in msg
    assert "basic:" in msg
