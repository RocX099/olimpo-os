...
def test_should_require_auth_can_be_forced_on_loopback(monkeypatch):
    monkeypatch.setenv("HERMES_DASHBOARD_FORCE_AUTH", "1")

    assert web_server.should_require_auth("127.0.0.1") is True
    assert web_server.should_require_auth("localhost") is True


def test_should_require_auth_keeps_loopback_open_by_default(monkeypatch):
...
def test_voice_status_requires_dashboard_session(loopback_client):
    r = loopback_client.get("/api/voice/status")

    assert r.status_code == 401


def test_voice_status_proxies_loopback_without_secrets(monkeypatch, loopback_client):
    loopback_client.headers[web_server._SESSION_HEADER_NAME] = web_server._SESSION_TOKEN

    def fake_voice_request(method, path, json_body=None):
        assert method == "GET"
    ...
    monkeypatch.setattr(web_server, "_voiceos_request", fake_voice_request)
    ...
    assert body["latest"]["text"] == "hola"
    assert "elevenlabs_api_key" not in body
    assert body["elevenlabs_configured"] in {True, False}
